...
def train():
    ...
    instructions = ''' 
            Please do the following steps before starting the ANet training:
            1. Download GRAB dataset and proceess the dataset using the /data/process_data.py.
            2. Set the grab-path, work-dir, and dataset-dir.
            3. Set the model-path to the folder containing SMPL-X body mdels.
            4. Change the CNet configuration file directly if you want to change the training configs (lr, batch_size, etc).  
                '''
    print(instructions)

    from configs.config_anet import conf as cfg

    parser = argparse.ArgumentParser(description='anet-Training')
